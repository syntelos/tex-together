...
function decompose {
    ...
    then
        if subject=$(echo "${input}" | sed 's%^[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9]-%%; s%-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9]\..*%%;') &&[ -n "${subject}" ]
        then
            if source=$(echo "${input}" | sed 's%^[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9]-%%; s%\.[A-Za-z_]*$%%; s%[A-Za-z_]*%%; s%^-%%; s%-$%%;') &&[ -n "${source}" ]
            then
